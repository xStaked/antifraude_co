      ...
      <div className="mx-auto max-w-2xl px-6 py-10 md:py-16">
        ...
        <div className="mt-6">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-border bg-card px-4 py-2">
            ...
            <span className="text-sm text-muted-foreground">Verificación en tiempo real</span>
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-foreground md:text-4xl">Consultar número</h1>
          <p className="mt-2 text-muted-foreground">
            Verifica si un número de celular ha sido reportado por la comunidad antes de realizar cualquier transacción.
          </p>
        </div>
        ...
        <div className="mt-8 rounded-2xl border border-border bg-card p-6 shadow-sm md:p-8">
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div>
              <label htmlFor="phone" className="block text-sm font-medium text-foreground">Número de celular</label>
              <div className="mt-3 flex gap-3">
                ...
                <Button type="submit" disabled={loading} size="lg" className="h-12 gap-2 bg-foreground px-5 text-background hover:bg-foreground/90">
                  {loading ? (
                    <>
        ...
        {result && (
          <div className="mt-6 space-y-4">
            <div className={`rounded-2xl border p-5 md:p-6 ${riskStyles[result.riskLevel] ?? "bg-card text-foreground border-border"}`}>
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-xl font-semibold md:text-2xl">{riskLabel[result.riskLevel] ?? result.riskLevel}</p>
                  <p className="mt-1 text-lg opacity-90">{result.displayPhone}</p>
                </div>
                <div className={`shrink-0 rounded-full border border-current/20 p-3 ${riskIconColor[result.riskLevel] ?? "text-muted-foreground"}`}>
            ...
            {(result.riskLevel === "none" || result.riskLevel === "low") && (
              <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-5">
                <div className="flex items-start gap-3">
                  <ShieldCheckIcon className="h-5 w-5 shrink-0 text-emerald-400" />
                  <div>
                    <p className="font-medium text-emerald-100">Resultado preventivo</p>
                    <p className="text-sm text-emerald-200/90">
                      Este número no tiene un historial grave de reportes. Aún así, verifica siempre el pago en tu app bancaria antes de entregar el producto.
                    </p>
                  </div>
                </div>
              </div>
            ...
            {result.recentReports.length > 0 && (
              <div className="rounded-2xl border border-border bg-card p-5 md:p-6">
                <h2 className="mb-4 text-lg font-semibold text-foreground">Incidentes recientes</h2>
                <ul className="space-y-3">
                  {result.recentReports.map((r) => (
                    <li key={r.id} className="flex flex-col gap-1 rounded-xl border border-border/60 bg-background/50 p-4 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        <p className="font-medium text-foreground">{r.fraudType}</p>
                        <p className="text-sm text-muted-foreground">{r.channel}</p>
                        {r.amountCents && (
                          <p className="text-sm text-accent">$ {(Number(r.amountCents) / 100).toLocaleString("es-CO")} COP</p>
                        )}
                      </div>
            ...
            <div className="rounded-xl border border-border bg-card/60 p-4 text-xs text-muted-foreground">{result.disclaimer}</div>
          </div>
        )}
      </div>
    </PageLayout>
